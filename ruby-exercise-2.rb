...
# for example 17 % 2 == 1
puts
  snowy_lines.each do |m|
    if m.split(split_pattern).count % 2 == 1
      puts ' ' + m
  end
end
...
while i < snowy_lines.count
  ...
  aa = snowy_lines[i + 1].split(split_pattern).last
  b = snowy_lines[i + 2].split(split_pattern).last
  bb = snowy_lines[i + 3].split(split_pattern).last
  c = snowy_lines[i + 4].split(split_pattern).last
  cc = snowy_lines[i + 5].split(split_pattern).last
  d = snowy_lines[i + 6].split(split_pattern).last
